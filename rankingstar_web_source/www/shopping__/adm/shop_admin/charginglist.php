...
$sql_common = " from {$g5['g5_shop_charging_table']} ";
...
<div class="btn_fixed_top">
    <a href="./chargingform.php" class="btn_01 btn">추가</a>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="th_loc">이미지</th>
        <th scope="col" id="th_st">시작일시</th>
        <th scope="col" id="th_end">종료일시</th>
        <th scope="col" id="th_hit">참여자</th>
        <th scope="col" id="th_mng">관리</th>
    </tr>
    </thead>
    <tbody>
    <?php
    $sql = " select * from {$g5['g5_shop_charging_table']} $sql_search
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if(file_exists($bimg)) {
            ...
            $bn_img .= '<img src="'.G5_DATA_URL.'/charging/'.$row['bn_id'].'" height="60" alt="'.get_text($row['bn_alt']).'">';
        }
        ...
        $res = sql_fetch("select count(0) as cnt from g5_shop_charging_check where bn_id = '{$row['bn_id']}'"); 
    ?>

    <tr class="<?php echo $bg; ?>">
        <td headers="th_id" class="td_num"><?php echo $row['bn_id']; ?></td>
        <td>
	        <?php if(date('y-m-d') < $bn_end_time):?>
	        <strong style="color:blue">진행중</strong>
	        <?php else:?>
	        <strong style="color:red">마감</strong>
	        <?php endif?>
        </td>
        <td headers="th_loc">
	        <?php echo $bn_img; ?>
        ...
        <td headers="th_mng" class="td_mng td_mns_m">
            <a href="./chargingform.php?w=u&amp;bn_id=<?php echo $row['bn_id']; ?>" class="btn btn_03">수정</a>
            <a href="./chargingformupdate.php?w=d&amp;bn_id=<?php echo $row['bn_id']; ?>" onclick="return delete_confirm(this);" class="btn btn_02">삭제</a>
        </td>
    </tr>
