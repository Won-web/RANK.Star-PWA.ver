...
<script type="text/javascript">
    $(document).ready(function() {
      clearPageNO(); // clearing page no of user controller
        ...
        function setDataTable() {
            $('#transactiondetailslist').DataTable({
                "paging": true,
                "lengthMenu": [ [10, 50, 100, 300], [10, 50, 100, 300] ],
                "searching": true,
                "ordering": true,
                "oLanguage": {
			...
			if(data == "승인 오류"){	
				...
				return "<font style='color:blue;'>승인 완료</font>";
			}else {
				return data;
			}
		         }},
      ...
      $(document).on('click','.deleteTransaction', function(e){
        ...
        e.preventDefault();
        $.confirm({
          title: '<?php echo lang('LBL_DELETE_TRANSACTION'); ?>',
          content: '<?php echo lang('LBL_DELETE_TRANSACTION_CONFIRMATION'); ?>',
          buttons: {
            confirm: {
            text: '<?php echo lang('BTN_OK');?>',
              action: function () {
                $.ajax({
                  url: "<?php echo base_url("transaction/deleteTransaction") ?>",
                  method: 'POST',
                  data: {
