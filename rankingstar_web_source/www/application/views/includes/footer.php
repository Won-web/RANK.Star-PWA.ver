...
function clearPageNO(){
  localStorage.removeItem('draw');
    ...
    function actionSuccessToastForDelete() {
        var toastmsg = "<?php echo $this->session->flashdata('success_msg_delete'); ?>";
        if(toastmsg != "" && toastmsg != undefined && toastmsg != null) {
            toastr.success(toastmsg);
        }
        $(toastmsg).addClass('deleteuser');
    }
