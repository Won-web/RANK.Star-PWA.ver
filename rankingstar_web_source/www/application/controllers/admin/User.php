...
class User extends MY_Controller
{
            ...
            if ($this->form_validation->run() === false) {
                ...
                if (!empty($isEmailExists)) {
                    ...
                    if (!empty($isMobileExists)) {
                        ...
                        $oauthUserInsert = $this->UserModel->upsertUserAuthData($email, md5($password));
                ...
                if (!empty($isEmailExists)) {
                    ...
                    if (!empty($isMobileExists)) {
                        ...
                        if($user_status === "deactive"){
                            $whereArr = array('user_id'=>$email);
                            if($login_type !== "auth"){
                                $whereCond = array('user_id'=>$user_id);
                                $userDetails = $this->UserModel->getByWhere('tbl_user', $whereCond);
                                $whereArr = array('user_id'=>$userDetails[0]['social_id']);
                            }
                            //Delete Data
                            $this->UserModel->delete('oauth_access_tokens', $whereArr);
                            $this->UserModel->delete('oauth_refresh_tokens', $whereArr);
    ...
    {
        $user_id = $this->uri->segment(4);
        $updateData = array('user_status' => 'deleted', 'email' => 'deleted_by_admin', 'mobile' => '00000000000', 'social_id' => '');
    ...
    public function deleteUserToUnsubscribe(){
        ...
        if(!empty($user_id)) {
            $this->db->trans_begin();
            $updateData = array(
		'email' => 'deleteduser@deleteduser.com',
		'mobile' => '01234567891',
                'user_status' => 'deleted'
            );
            $whrUserArr = array(
