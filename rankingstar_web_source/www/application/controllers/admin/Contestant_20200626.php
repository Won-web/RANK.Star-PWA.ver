...
class Contestant extends MY_Controller
{
    ...
    public function contestantlist()
    {
        ...
        $sortColumn = ['tcm.current_ranking', 'tc.contest_name', 'tcd.name', 'total_voting'];
    ...
    public function savecontestant()
    {
        ...
        $this->form_validation->set_rules('mobile', lang('PHONE_NO'), 'required', array('required' => '%s ' . lang('VALIDATION_REQUIRED')));
            ...
            if ($userId) {
                ...
                if ($ContestantId) {
                    ...
                    if (!$this->upload->do_upload('main_image')) {
                        $this->session->set_flashdata('file_upload_error', lang('IMAGE_VALIDATION_1'));
                        redirect(BASE_URL . 'contestant/addcontestant');
                    ...
                    if (!$this->upload->do_upload('thumb_image')) {
                        $this->session->set_flashdata('file_upload_error', lang('IMAGE_VALIDATION_2'));
                        redirect(BASE_URL . 'contestant/addcontestant');
                        $userWhereArr = array('user_id' => $userId);
                        $contestantWhrArr = array('contestant_id' => $ContestantId);
                        $this->ContestantModel->delete('tbl_user' , $userWhereArr);
