...
class User extends MY_Controller
{
    ...
    public function userlist()
    {
        ...
        // $order_by=null;
        $totalData = $this->UserModel->getUserData($searchText, $order_by, $sort_by, $offset, 0);
        $userData = $this->UserModel->getUserData($searchText, $order_by, $sort_by, $offset, $limit);
        //Process Data
        $data = array();
    
        foreach ($userData as $user) {
            $count ++;
            $details = '<a data-toggle="tooltip" title="Details" href="' . BASE_URL . 'user/details/' . $user['user_id'] . '" data-id="' . $user['user_id'] . '">' . $user['name'] . '</a>';
            $delete = '';
            $sessionData = $this->auth->getUserSession();
            if ($sessionData['user_type'] === "admin"){
              $delete = '<a data-toggle="tooltip" title="' . lang('TT_REMOVE_USER') . '" class="remove-user" data-url="' . BASE_URL . 'user/delete/' . $user['user_id'] . '" href="#" data-id="' . $user['user_id'] . '"><i class="fas fa-user-times"></i></a>';
            }
            $data[] = array($count,$details, $user['email'], $user['mobile'], date('Y-m-d H:i:s', strtotime($user['register_date_time'])), $user['user_status'], $delete);
    ...
    public function adduser()
    {
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('email', lang('LBL_EMAIL'), 'required|valid_email|is_unique[tbl_user.email]', array('required' => '%s ' . lang('VALIDATION_REQUIRED'), 'valid_email' => lang('VALIDATION_EMAIL'), 'is_unique' => lang('MSG_EMAIL_EXISTS')));
    ...
    public function edituser()
    {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() === false) {
                ...
                $isEmailExists = array();
                if($login_type === "auth") {
                    $whereArr = array('user_id !=' => $user_id, 'email' => $email);
    ...
    public function details()
    {
        ...
        $this->load->view('user_details_view', $data);
    }
    ...
    public function delete()
    {
        ...
        if ($isDeleted) {
            ...
            if ($userDetail[0]['user_type'] === 'contestant') {
                ...
                $contestantMappingArr = $this->UserModel->getByWhere('tbl_contestant_mapping', 
                    $contestantWhereCon);
                // update status to deleted
                $contestantMapWhere = array(
                    'contestant_id' => $contestantDetails[0]['contestant_id']
                );
                $contestantMapdelete = $this->UserModel->update('tbl_contestant_mapping', 
                    $updateContestantWhr, $contestantMapWhere);
    ...
    public function purchasehistory(){
     ...
     public function usagehistory(){
        ...
        foreach ($purchaseData['voting_list'] as $key => $value) {
            ...
            $purchaseData['voting_list'][$key]['vote_date'] =  date('Y-m-d H:i:s',strtotime($value['vote_date']));
