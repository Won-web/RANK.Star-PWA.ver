...
$od_b_name        = clean_xss_tags($od_name);
$od_b_tel         = clean_xss_tags($od_tel);
$od_b_hp          = clean_xss_tags($od_hp);
$od_b_addr1       = clean_xss_tags($od_addr1);
$od_b_addr2       = clean_xss_tags($od_addr2);
$od_b_addr3       = clean_xss_tags($od_addr3);
$od_b_addr_jibeon = preg_match("/^(N|R)$/", $od_addr_jibeon) ? $od_addr_jibeon : '';
...
//첫 주문시 회원정보에 배송지반영
$sql_mb = " update g5_member 
                      set mb_zip1 = '{$_POST['od_zip']}',
                          mb_zip2 = '2',
                          mb_addr1 = '{$_POST['od_addr1']}',
                          mb_addr2 = '{$_POST['od_addr2']}',
                          mb_addr3 = '{$_POST['od_addr3']}',
                          mb_addr_jibeon = '{$_POST['od_addr_jibeon']}',
                          mb_hp = '{$_POST['od_hp']}',
                          mb_tel = '{$_POST['od_tel']}',
                          mb_email = '{$_POST['od_email']}'
                    where mb_id = '{$member['mb_id']}' ";
sql_query($sql_mb);
...
//insert_point($member['mb_id'], $s_point, "주문번호 {$od_id} 구매완료",'','','','','PG','starshop');
...
$sql_od = " update {$g5['g5_shop_order_table']}
            set od_status  = '완료'
            where od_id = '$od_id' ";
sql_query($sql_od);


save_order_point("완료",1);*/
...
save_order_point("입금",$it_type,$amount);
...
if($it_1 == '1'){
	goto_url(G5_SHOP_URL.'/orderinquiryview.php?od_id='.$od_id.'&amp;uid='.$uid.'&it_1=1');
} else {
