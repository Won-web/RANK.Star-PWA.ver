...
// 포인트 부여
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0,$desc,$type,$amount='')
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    // 포인트 UPDATE
    $sql_pp = " update {$g5['member_table']} set mb_point = '$po_mb_point' where mb_id = '$mb_id' ";
    sql_query($sql_pp);
    
    //어플에 포인트 넘기기
    $sql_app = " insert into tbl_star_purchase
                        set user_id = '{$mb_id}',
	                        star = '{$point}',
	                        description = '{$desc}',
	                        type = '{$type}',
...
// 포인트 삭제
function delete_point($mb_id, $rel_table, $rel_id, $rel_action,$desc='Shop',$type='starshop',$point,$amount=0)
{
    global $g5;
...
// 광고 포인트 콜백 포인트 부여
function insert_callback_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0,$desc,$type,$amount='')
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { 
        ...
        $rs = sql_fetch(" select tu.email as email , tu.mobile as mobile, tud.name as name 
                            FROM tbl_user AS tu 
                                LEFT JOIN tbl_user_details AS tud 
                                    ON tu.user_id = tud.user_id 
                            WHERE tu.user_id = $mb_id" );
        if (!$rs['email']) {
            return 0;
        }else{
            $mb_name = $rs['name'];
            ...
            $insertQry = " insert into {$g5['member_table']}
                    set mb_id = '{$mb_id}',
