...
function get_coupon_id()
{
    ...
    $str = preg_replace("/([0-9A-Z]{4})([0-9A-Z]{2})([0-9A-Z]{4})/", "\\1-\\2-\\3", $str);
...
//------------------------------------------------------------------------------
function save_order_point($ct_status="입금",$it_1,$amount='')
{
    global $g5, $default;
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($od_row['mb_id'] && $row['ct_point'] > 0) { // 회원이면서 포인트가 0보다 크다면
            ...
            if($it_1 == 1){
	            $po_content = "주문번호 {$od_row['od_id']} ({$row['ct_id']}) 포인트 지급";
            } else {
	            $po_content = "주문번호 {$od_row['od_id']} ({$row['ct_id']}) 상품구매";
            }
            insert_point($od_row['mb_id'], $po_point, $po_content, "@delivery", $od_row['mb_id'], "{$od_row['od_id']},{$row['ct_id']}",'','Shop','starshop',$amount);
        }
