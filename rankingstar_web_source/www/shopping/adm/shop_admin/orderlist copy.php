...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td headers="th_ordnum" class="td_odrnum2" rowspan="2" colspan="2">
            ...
            <a href="#non" onclick="window.open('./orderprintresult2.php?od_id=<?=$od_id?>&ct_status=<?=$row['od_status']?>', 'google', 'width=500,height=500');"><?php echo $disp_od_id; ?></a>
            <?php echo $od_mobile; ?>
            <?php echo $od_paytype; ?>
            ...
            <?php
	        $od_id = $row['od_id'];
            $sql_item = " select * from {$g5['g5_shop_order_table']} where od_id = '{$od_id}' ";
			$od = sql_fetch($sql_item);
			
			// 상품목록
			$sql_item = " select it_id,
			                it_name,
			                cp_price,
			                ct_notax,
			                ct_send_cost,
			...
			<?php
				for($i=0; $row_view=sql_fetch_array($result_item); $i++) {
					
					// 상품이미지
            $image = get_it_image($row_view['it_id'], 30, 30);

            // 상품의 옵션정보
            $sql_view = " select ct_id, it_id, ct_price, ct_qty, ct_option, ct_status, cp_price, ct_send_cost, io_type, io_price
                        from {$g5['g5_shop_cart_table']}
                        where od_id = '$od_id'
                          and it_id = '{$row_view['it_id']}'
                        order by io_type asc, ct_id asc ";
            $res = sql_query($sql_view);
            
            ?>
            
            <div style="margin:-5px;margin-top:5px;padding:8px;">
			<?php echo $image; ?> <span style="padding-left:5px"><?php echo stripslashes($row_view['it_name']); ?></span>
            </div>
