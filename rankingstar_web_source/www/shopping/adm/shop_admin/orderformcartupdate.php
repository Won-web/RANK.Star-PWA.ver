...
for ($i=0; $i<$cnt; $i++)
{
    ...
    if ($mb_id && $ct['ct_point'] && $ct['ct_point_use'])
    {
        ...
        //이것. 문제시 삭제 할 것
        //insert_point($mb_id, (-1) * ($ct[ct_point] * $ct[ct_qty]), "주문번호 $od_id ($ct_id) {$ct_status}",'','','','','PG취소','starshop');
        
		if($ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절'){
			$iamount = -$ct['ct_price'] * $ct['ct_qty'];
		} else {
			$iamount = $ct['ct_price'] * $ct['ct_qty'];
		}
        delete_point($mb_id, "@delivery", $mb_id, "$od_id,$ct_id",'Shop'.$ct_status,'starshop',(-1) * ($ct[ct_point] * $ct[ct_qty]),$iamount);
    }
    }
    ...
    // it_id를 배열에 저장
    if($ct_status == '주문' || $ct_status == '취소' || $ct_status == '반품' || $ct_status == '품절' || $ct_status == '입금')
        $arr_it_id[] = $ct['it_id'];
}
...
if(is_array($arr_it_id) && !empty($arr_it_id)) {
    ...
    foreach($unq_it_id as $it_id) {
        $sql2 = " select sum(ct_qty) as sum_qty from {$g5['g5_shop_cart_table']} where it_id = '$it_id' and ct_status = '입금' ";
        $row2 = sql_fetch($sql2);
